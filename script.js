...
// Local logo lookup. Primary source is logos/manifest.js (generated by
// scripts/fetch-logos.mjs — every beer has a hardcoded file there). A per-beer
// `logo:"logos/<file>"` field on a beers[] entry still wins for one-off
// overrides. The Brandfetch → favicon chain remains as runtime fallback if a
// local file is missing.
const LOCAL_LOGOS={};
function rebuildLocalLogos(){
  for(const k in LOCAL_LOGOS) delete LOCAL_LOGOS[k];
  const manifest=(typeof window!=='undefined' && window.LOCAL_LOGOS_MANIFEST)||{};
  for(const k in manifest) LOCAL_LOGOS[k]=manifest[k];
