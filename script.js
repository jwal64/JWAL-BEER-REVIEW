...
const breweries=[
  {name:"Weihenstephaner",        location:"Freising, Bavaria",         country:"Germany",     cc:"DE", beers:"Weihenstephaner · Müchner Weisse · Munchen Dunkel", lat:48.3953,lng:11.7291, ratings:[4.50,4.75,2.75]},
  ...
  {name:"Heineken",               location:"Amsterdam, Noord-Holland",  country:"Netherlands", cc:"NL", beers:"Heineken",                                          lat:52.3578,lng:4.8918,  ratings:[3.25,3.25]},
  ...
  {name:"Anheuser-Busch",         location:"St. Louis, Missouri",       country:"USA",         cc:"US", beers:"Budweiser · Bud Light · Michelob Ultra",            lat:38.6072,lng:-90.2124, ratings:[3.00,3.00,2.50]},
  {name:"Molson Coors",           location:"Golden, Colorado",          country:"USA",         cc:"US", beers:"Coors Light",                                       lat:39.7555,lng:-105.2211,ratings:[3.00,2.75]},
  {name:"Grupo Modelo",           location:"Mexico City, CDMX",         country:"Mexico",      cc:"MX", beers:"Modelo · Modelo Negra · Corona Extra",              lat:19.4274,lng:-99.1677, ratings:[3.25,2.25,3.00,3.00,3.75]},
  ...
  {name:"Unibroue",               location:"Chambly, Quebec",           country:"Canada",      cc:"CA", beers:"La Fin Du Monde",                                   lat:45.4412,lng:-73.2615, ratings:[2.75,3.75]},
  {name:"Kronenbourg",            location:"Obernai, Alsace",           country:"France",      cc:"FR", beers:"1664",                                              lat:48.4637,lng:7.4845,  ratings:[3.00]},
  {name:"Sapporo Brewery",        location:"Sapporo, Hokkaido",         country:"Japan",       cc:"JP", beers:"Sapporo",                                           lat:43.0685,lng:141.3544, ratings:[3.50,3.00]},
  ...
  {name:"Estrella Galicia",       location:"A Coruña, Galicia",         country:"Spain",       cc:"ES", beers:"Estrella Galicia",                                   lat:43.3623,lng:-8.4115,  ratings:[4.25]},
...
// HELPERS
// ══════════════════════════════════════════════════════════════
const sC={"Wheat Beer-Hefeweizen":"#ff6600","Belgian Strong Golden Ale":"#ffaa00","Stout-Irish Dry":"#888","Lager-Pale":"#00cc44","Lager-Mexican":"#00aa33","Pilsner-Other":"#00aaff","Belgian Tripel":"#bb44ff","Lager-Munich Dunkel":"#ff4400","Lager-American":"#555","Lager-American Light":"#444","Lager-Helles":"#22bb77","Belgian Strong Dark Ale":"#8b0000","Witbier":"#f0c040","Brown Ale-English":"#8b4513"};
...
(function(){
  const tabMap={
    ...
    'q':'contrarian','w':'ipo','e':'audit','r':'comments',
    't':'beerentry'
  };
...
function showTab(id,btn){
  ...
  if(id==='comments'){ populateBeerDropdown(); renderComments(); }
  if(id==='beerentry'){ if(!isUnlocked()){openPwModal('beerentry');return;} renderBeerEntries(); }
...
function renderComments(){
  ...
  if(!arr.length){
    el.innerHTML='<div style="color:#333;text-align:center;padding:30px;font-size:9px">NO NOTES YET — ADD ONE ABOVE</div>';
    return;
  }
  el.innerHTML=arr.slice().reverse().map((c,i)=>`
    <div style="display:flex;gap:10px;align-items:flex-start;padding:8px 0;border-bottom:1px solid #1a1a1a">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:9px;color:#333;margin-top:3px">${c.name?'<span style="color:#888">'+escHtml(c.name)+'</span> — ':''} ${c.ts}</div>
      </div>
      <button onclick="deleteComment(${arr.length-1-i})" style="background:none;border:none;color:#333;font-family:var(--mono);font-size:10px;cursor:pointer;flex-shrink:0;padding:0 4px" onmouseover="this.style.color='#ff2222'" onmouseout="this.style.color='#333'">✕</button>
    </div>`).join('');
}
function escHtml(s){
...
let BLOCKED_WORDS=[];
(async function(){
  const _k='jwal-brew-terminal-filter';
  const _iv='ebad6626a1e83ed2ca00ee5b08e13812';
  const _d='';
  const enc=new TextEncoder();
  const keyData=await crypto.subtle.digest('SHA-256',enc.encode(_k));
  const key=await crypto.subtle.importKey('raw',keyData,{name:'AES-CBC'},false,['decrypt']);
  const iv=new Uint8Array(_iv.match(/.{2}/g).map(b=>parseInt(b,16)));
  const buf=new Uint8Array(_d.match(/.{2}/g).map(b=>parseInt(b,16)));
  const dec=await crypto.subtle.decrypt({name:'AES-CBC',iv},key,buf);
  BLOCKED_WORDS=JSON.parse(new TextDecoder().decode(dec));
})();
...
function populateBeerDropdown(){
  const sel=document.getElementById('comment-beer');
  if(!sel) return;
  const existing=sel.options.length;
  if(existing>1) return;
  const unique=[...new Set(beers.map(b=>b.beer))].sort();
  unique.forEach(name=>{
...
function addComment(){
  ...
  const beer=document.getElementById('comment-beer').value;
  ...
  if(!beer){
    alert('⚠ Please select a beer from the dropdown.');
    document.getElementById('comment-beer').focus();
    return;
  }
  if(!txt) return;
  if(containsBadWords(name+' '+txt)){
    alert('⚠ Your comment contains inappropriate language. Please revise.');
    return;
  }
  const arr=loadComments();
  arr.push({name:name,subject:beer,text:txt,ts:new Date().toLocaleString()});
  saveComments(arr);
...
// BEER ENTRY
// ══════════════════════════════════════════════════════════════
function loadUserBeers(){
  return JSON.parse(localStorage.getItem('brewUserBeers')||'[]');
}
function saveUserBeers(arr){
  localStorage.setItem('brewUserBeers',JSON.stringify(arr));
}
function initBeerEntryForm(){
  // Populate style datalist
  const styles=[...new Set(beers.map(b=>b.style))].sort();
  const dl=document.getElementById('style-list');
  if(dl) dl.innerHTML=styles.map(s=>`<option value="${s}">`).join('');
  // Populate origin and cc dropdowns
  const codes=Object.keys(CNAMES).sort((a,b)=>(CNAMES[a]||a).localeCompare(CNAMES[b]||b));
  const originSel=document.getElementById('entry-origin');
  const ccSel=document.getElementById('entry-cc');
  codes.forEach(c=>{
    const lbl=`${FLAGS[c]||''} ${CNAMES[c]||c} (${c})`;
    if(originSel) originSel.innerHTML+=`<option value="${c}">${lbl}</option>`;
    if(ccSel) ccSel.innerHTML+=`<option value="${c}">${lbl}</option>`;
  });
}
function renderBeerEntries(){
  const arr=loadUserBeers();
  const cnt=document.getElementById('entry-count');
  const scnt=document.getElementById('entry-saved-count');
  if(cnt) cnt.textContent=arr.length+' ENTR'+(arr.length===1?'Y':'IES');
  if(scnt) scnt.textContent=arr.length+' SAVED';
  const el=document.getElementById('entry-list');
  if(!arr.length){
    el.innerHTML='<div style="color:#333;text-align:center;padding:30px;font-size:9px">NO ENTRIES YET — ADD ONE ABOVE</div>';
    return;
  }
  const MONTHS={Jan:1,Feb:2,Mar:3,Apr:4,May:5,Jun:6,Jul:7,Aug:8,Sep:9,Oct:10,Nov:11,Dec:12};
  el.innerHTML=`<table style="width:100%;border-collapse:collapse;font-size:10px">
    <thead><tr style="border-bottom:1px solid #333">
      <th style="text-align:left;padding:4px 6px;color:#ff6600">BEER</th>
      <th style="text-align:left;padding:4px 6px;color:#ff6600">STYLE</th>
      <th style="text-align:center;padding:4px 6px;color:#ff6600">ORIGIN</th>
      <th style="text-align:center;padding:4px 6px;color:#ff6600">ABV</th>
      <th style="text-align:center;padding:4px 6px;color:#ff6600">METHOD</th>
      <th style="text-align:left;padding:4px 6px;color:#ff6600">CITY</th>
      <th style="text-align:center;padding:4px 6px;color:#ff6600">RATING</th>
      <th style="text-align:center;padding:4px 6px;color:#ff6600">DATE</th>
      <th style="text-align:center;padding:4px 6px;color:#ff6600"></th>
    </tr></thead><tbody>`
    +arr.map((b,i)=>{
      const rc=b.rating>=4?'#00cc44':b.rating>=3?'#ff6600':'#ff2222';
      return `<tr style="border-bottom:1px solid #1a1a1a">
        <td style="padding:5px 6px;color:#e8e8e8">${escHtml(b.beer)}</td>
        <td style="padding:5px 6px;color:#888">${escHtml(b.style)}</td>
        <td style="text-align:center;padding:5px 6px">${FLAGS[b.origin]||''} ${b.origin}</td>
        <td style="text-align:center;padding:5px 6px;color:#888">${b.abv}%</td>
        <td style="text-align:center;padding:5px 6px;color:#888">${b.method}</td>
        <td style="padding:5px 6px;color:#888">${escHtml(b.city)}</td>
        <td style="text-align:center;padding:5px 6px;color:${rc};font-weight:700">${b.rating.toFixed(2)}</td>
        <td style="text-align:center;padding:5px 6px;color:#555">${b.month} ${b.year}</td>
        <td style="text-align:center;padding:5px 6px">
          <button onclick="deleteBeerEntry(${i})" style="background:none;border:none;color:#333;font-family:var(--mono);font-size:10px;cursor:pointer;padding:0 4px" onmouseover="this.style.color='#ff2222'" onmouseout="this.style.color='#333'">✕</button>
        </td>
      </tr>`;
    }).join('')
    +`</tbody></table>`;
}
function addBeerEntry(){
  const errEl=document.getElementById('entry-err');
  const okEl=document.getElementById('entry-ok');
  errEl.style.display='none'; okEl.style.display='none';

  const beer=document.getElementById('entry-beer').value.trim();
  const style=document.getElementById('entry-style').value.trim();
  const origin=document.getElementById('entry-origin').value;
  const abv=parseFloat(document.getElementById('entry-abv').value);
  const method=document.getElementById('entry-method').value;
  const rating=parseFloat(document.getElementById('entry-rating').value);
  const isNew=document.getElementById('entry-isNew').checked;
  const city=document.getElementById('entry-city').value.trim();
  const region=document.getElementById('entry-region').value.trim();
  const country=document.getElementById('entry-country').value.trim();
  const cc=document.getElementById('entry-cc').value;
  const month=document.getElementById('entry-month').value;
  const year=parseInt(document.getElementById('entry-year').value);

  // Validation
  if(!beer||!style||!origin||isNaN(abv)||!method||isNaN(rating)||!city||!region||!country||!cc||!month||isNaN(year)){
    errEl.textContent='✗ ALL REQUIRED FIELDS MUST BE FILLED';
    errEl.style.display='block';
    return;
  }
  if(rating<1||rating>5){
    errEl.textContent='✗ RATING MUST BE BETWEEN 1.0 AND 5.0';
    errEl.style.display='block';
    return;
  }

  const MONTHS={Jan:1,Feb:2,Mar:3,Apr:4,May:5,Jun:6,Jul:7,Aug:8,Sep:9,Oct:10,Nov:11,Dec:12};
  const entry={beer,style,origin,abv,method,city,region,country,cc,rating,isNew,month,monthN:MONTHS[month]||1,year};
  ...
  // Also add to in-memory beers array for current session
  beers.push(entry);

  // Clear form
  document.getElementById('entry-beer').value='';
  document.getElementById('entry-style').value='';
  document.getElementById('entry-origin').value='';
  document.getElementById('entry-abv').value='';
  document.getElementById('entry-rating').value='';
  document.getElementById('entry-isNew').checked=false;
  document.getElementById('entry-city').value='';
  document.getElementById('entry-region').value='';
  document.getElementById('entry-country').value='';
  document.getElementById('entry-cc').value='';

  okEl.textContent='✓ BEER ENTRY SAVED — '+beer.toUpperCase()+' ('+rating.toFixed(2)+'/5)';
  okEl.style.display='block';
  setTimeout(()=>okEl.style.display='none',4000);

  renderBeerEntries();
}
function deleteBeerEntry(idx){
  const arr=loadUserBeers();
  const removed=arr.splice(idx,1)[0];
  saveUserBeers(arr);
  // Remove from in-memory beers array
  if(removed){
    const bi=beers.findIndex(b=>b.beer===removed.beer&&b.style===removed.style&&b.rating===removed.rating&&b.city===removed.city&&b.month===removed.month&&b.year===removed.year);
    if(bi!==-1) beers.splice(bi,1);
  }
  renderBeerEntries();
}
